...
| Datos de la vivienda                     | P8520S1      | Energía eléctrica                           | Discreta | Numérica    |
...
| Servicios del hogar                      | I_UGASTO     | Ingreso mensual total de la unidad de gasto | Discreta | Numérica    |
...
vivienda_cols <- c('ï..DIRECTORIO', 'REGION', 'P8520S1', 'P8520S1A1')
servicios_cols <- c('ï..DIRECTORIO', 'SECUENCIA_ENCUESTA', 'I_HOGAR', 'I_UGASTO')
...
clean_input <- all_input %>% select(-c("ï..DIRECTORIO", "SECUENCIA_P", "SECUENCIA_ENCUESTA"))

write.csv(all_input,"../db/input.csv", row.names = FALSE)

labeled_input <- all_input %>% left_join(labeled, by = c('ï..DIRECTORIO', 'SECUENCIA_P'))

clean_labeled_input <- labeled_input %>% select(-c("ï..DIRECTORIO", "SECUENCIA_P", "SECUENCIA_ENCUESTA"))

write.csv(all_input,"../db/labeled_input.csv", row.names = FALSE)
```
