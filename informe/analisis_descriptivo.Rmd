...
<!-- # Base de datos:  -->
<!-- La base de datos fue tomada de la Encuesta Nacional de Calidad de Vida en Colombia [@DANE2020a] y después de un proceso de limpieza y depuración de los datos, hemos seleccionado las siguientes variables de interés de acuerdo con la literatura [@Robayo2019]:  -->
...
table_f <- function(data, colname) {
  ...
  len=length(frec)
  return(kbl(frecuency)%>%kable_paper(bootstrap_options = "striped", full_width = F,position = "float_right")
         %>%column_spec(1:3, bold = F, border_right = F, color = "#273746")
         %>%row_spec(1:nrow(frecuency), bold = F, color = "white", background = "#273746"))
}
...
A continuación se muestra el conteo de la variable sexo con 1 **hombre** y 2 **mujer** mostrando una variable dicotómica con una muestra de mas hombres que mujeres y un total de 93988 encuestados cabeza de hogar. Esta es una variable de las más influyentes según la documentación encontrada ya que muchos sugieren el estudio de la fertilidad de las mujeres para predecir el crecimiento poblacional.
