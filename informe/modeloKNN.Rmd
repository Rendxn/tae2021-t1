...
knitr::kable(knn_fit$results) %>% kable_material_dark()
```
...
knitr::kable(knn_fit$results[2,]) %>% kable_material_dark()
```
...
RMSE(pred = test_pred, obs = test_clean$HIJOS)
```
...
```{r}
R2(pred = test_pred, obs = test_clean$HIJOS)
```

Este coeficiente representa la proporción de varianza en los resultados que nuestro modelo es capaz de explicar y nos da información sobre qué tan bien se ajusta el modelo a los datos.