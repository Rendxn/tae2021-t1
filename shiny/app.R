...
ui <-  navbarPage(
    ...
    tabPanel("Modelo", sidebarLayout(
        ...
        mainPanel(
            ...
            p(
                'Basados en los datos dispuestos por el DANE en el Archivo Nacional de Datos sobre la Encuesta Nacional de Calidad de Vida del 2019, entrenamos el modelo K Nearest Neighbours (KNN) para predecir el número de hijos en un hogar colombiano.'
            ),
            hr(),
            # predicción
            uiOutput('prediction')
...
server <- function(input, output) {
    ...
    output$prediction <- renderUI({
        ...
        validate(
            ...
            need(
                input$ingreso >= 0,
                'Digite un ingreso mensual total válido.'
            ),
            need(
                input$cant_personas > 0,
                'Ingrese una cantidad de personas en el hogar válido'
            )
        )
        ...
        entrada$CANT_PERSONAS_HOGAR <-
            as.integer(entrada$CANT_PERSONAS_HOGAR)
        # predicción
